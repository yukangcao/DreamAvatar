...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        def sigma_fn(t_starts, t_ends, ray_indices, idx=batch_idx):
            ...
            if self.training:
                sigma = self.geometry.forward_density(positions, idx=batch_idx)[..., 0]
            else:
                sigma = chunk_batch(
        ...
        if self.training:
            geo_out = self.geometry(
                positions, output_normal=True, idx=batch_idx
            )
